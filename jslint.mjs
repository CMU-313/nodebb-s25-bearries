...
let jslint_rgx_cap = /^[A-Z]/;
let jslint_rgx_crlf = /\n|\r\n?/;
let jslint_rgx_digits_bits = /^[01_]*/;
let jslint_rgx_digits_decimals = /^[0-9_]*/;
let jslint_rgx_digits_hexs = /^[0-9A-F_]*/i;
let jslint_rgx_digits_octals = /^[0-7_]*/;
let jslint_rgx_directive = /^(jslint|property|global)\s+(.*)$/;
let jslint_rgx_directive_part =
  /([a-zA-Z$_][a-zA-Z0-9$_]*)(?::\s*(true|false))?,?\s*|$/g;
let jslint_rgx_identifier = /^([a-zA-Z_$][a-zA-Z0-9_$]*)$/;
...
let jslint_rgx_mega =
  ...
  /[\u0060\\]|\$\{/;
let jslint_rgx_module = /^[a-zA-Z0-9_$:.@\-\/]+$/;
let jslint_rgx_numeric_separator_illegal = /__|_$|_n$/m;
let jslint_rgx_slash_star_or_slash = /\/\*|\/$/;
let jslint_rgx_tab = /\t/g;
let jslint_rgx_todo = /\b(?:todo|TO\s?DO|HACK)\b/;
...
let jslint_rgx_url_search_window_jslint = /[&?]window_jslint=1(?:$|&)/m;
let jslint_rgx_weird_property = /^_|\$|Sync$|_$/m;
...
function assertOrThrow(condition, message) {
  ...
  if (!condition) {
    throw !message || typeof message === "string"
...
function globExclude({
  ...
  function globAssertNotWeird(list, name) {
    ...
    [
      ["\n", /^.*?([\u0000-\u0007\r]).*/gm],
      ["\r", /^.*?([\n]).*/gm],
    ].forEach(function ([separator, rgx]) {
  ...
  function globToRegexp(pattern) {
    ...
    pattern = pattern.replace(/\*\*\*+/g, "**");
    pattern.replace(/\\\\|\\\[|\\\]|\[|\]|./g, function (match0) {
      switch (match0) {
        case "[":
          if (isClass) {
            strClass += "[";
    ...
    strRegex = strRegex.replace(/(^|\/)\\\*\\\*(\/|$)/gm, "$1.*?$2");
    ...
    strRegex = strRegex.replace(/\/$/gm, "\\/.*?");

    // Merge strClass into strRegex.

    ii = 0;
    strClass = strClass.split("\u0000");
    strRegex = strRegex.replace(/\u0000/g, function () {
      ii += 1;
      if (strClass[ii] === "") {
        return "";
      }
  ...
  if (includeList.length > 0) {
    ...
    pathnameList = pathnameList.replace(/^[^\u0000].*/gm, "");
    pathnameList = pathnameList.replace(/^\u0000+/gm, "");
  }
...
function jslint(
  ...
  function is_equal(aa, bb) {
    ...
    if (aa.arity === bb.arity && aa.id === bb.id) {
      if (aa.id === "." || aa.id === "?.") {
        ...
        return (
          is_equal(aa.expression, bb.expression) && is_equal(aa.name, bb.name)
        );
      }
      if (aa.arity === "unary") {
        // test_cause:
      ...
      cause_dict[
        JSON.stringify([
          String(new Error().stack)
            ...
            .replace(/^Object\./, ""),
          code,
          String(aa === undefined || aa === token_global ? "" : aa),
          column || 0,
        ])
      ] = true;
    }
  ...
  function warn_at(code, line, column, a, b, c, d) {
    ...
    switch (code) {
      ...
      case "expected_type_string_a":
        ...
        break;
      case "freeze_exports":
        mm = `Expected 'Object.freeze('. All export values should be frozen.`;
        break;
      ...
      case "unexpected_typeof_a":
        mm = `Unexpected 'typeof'. Use '===' to compare directly with ${a}.`;
        break;
  ...
  try {
    ...
    jslint_assert(function_stack.length === 0, `function_stack.length === 0.`);
  ...
  warning_list
    ...
    .map(function (
      { column, line, line_source, message, stack_trace = "" },
      ii,
      list,
    ) {
...
<li>
    ...
    // init signature
    source = source.replace(/(\([\S\s]*?\)) \{/, function (match0, match1) {
...
<li>Example usage:<pre class="apidocCodePre">${example}</pre></li>
  ...
  function trim_start(str) {
    ...
    let whitespace = "";
    str.trim().replace(/^ */gm, function (match0) {
  ...
  example_list = await Promise.all(
    example_list.map(async function (file) {
  ...
  module_list = await Promise.all(
    module_list.map(async function ({ pathname }) {
      ...
      return {
        elem_list: Object.keys(moduleObj)
          .map(function (key) {
          ...
          .map(function (elem) {
            elem_ii += 1;
            elem.signature = elem.signature.replace(">", ">" + elem_ii + ". ");
            elem.source = elem.source.replace('">', '">' + elem_ii + ". ");
            return elem;
          }),
...
<div class="apidocSectionDiv">
    ...
    ` +
    module_list
      .map(function ({ elem_list, id, moduleName }) {
        return (
          `
        <li>
...
async function jslint_cli({
  ...
  function jslint_from_file({
    ...
    if (mode_conditional && !/^\/\*jslint\b/m.test(code.slice(0, 65536))) {
      return;
    }
      ...
      console_error(
        mode_wrapper_vim
          ? // PR-349 - Print warnings in format readable by vim.
            ...
            result_from_file.warnings
              .slice(0, 10)
              .map(function ({ column, line, message }, ii) {
                return (
                  file + ":" + ii + ":" + line + ":" + column + ":" + message
                );
              })
              .join("\n")
          : // Print warnings in format readable by human.
            ...
            "\u001b[1mjslint " +
              ...
              result_from_file.warnings
                .slice(0, 10)
                .map(function ({ formatted_message }) {
            ...
            empty(),
            {
              beta: Boolean(
                process_env.JSLINT_BETA &&
                  !/0|false|null|undefined/.test(process_env.JSLINT_BETA),
              ),
              node: true,
            },
            option,
  ...
  if (
    !(
      ...
      (
        ...
        (/[\/|\\]jslint(?:\.[cm]?js)?$/m.test(process_argv[1]) || mode_cli) &&
  ...
  switch (command[0]) {
    ...
    case "jslint_apidoc":
      await jslint_apidoc(
        Object.assign(JSON.parse(process_argv[3]), {
          pathname: command[1],
        }),
      );
      return;
...
function jslint_phase2_lex(state) {
  ...
  function check_numeric_separator(digits, column) {
    ...
    digits.replace(jslint_rgx_numeric_separator_illegal, function (ignore, ii) {
      // test_cause:
  ...
  function lex_comment() {
    ...
    if (snippet === "//") {
      ...
      jj = line_source.slice(0, ii).search(jslint_rgx_slash_star_or_slash);
      if (jj >= 0) {
        // test_cause:
        // ["/*/**/", "lex_comment", "nested_comment", "", 2]
    ...
    [the_comment.directive, body] = Array.from(
      snippet.match(jslint_rgx_directive) || [],
    ).slice(1);
    ...
    ii = 0;
    body.replace(jslint_rgx_directive_part, function (match0, key, val, jj) {
      if (ii !== jj) {
        // test_cause:
  ...
  function lex_token() {
    ...
    while (true) {
      ...
      if (!match) {
        ...
        return stop_at("unexpected_char_a", line, column, line_source[0]);
      }
      snippet = match[1];
...
*/
  ...
  function read_digits(base, mode) {
    ...
    if (mode === mode_digits_numeric_separator) {
      ...
      warn_at("illegal_num_separator", line, column + digits.indexOf("_") + 1);
    }
  ...
  function token_create(id, value, identifier) {
    ...
    switch (id) {
      ...
      case ")":
        ...
        break;
      case "=>":
        if (token_prv_expr.id === ")" && paren_backtrack_list[paren_depth]) {
...
function jslint_phase3_parse(state) {
  ...
  function check_left(left, right) {
    ...
    if (
      !left.identifier &&
      (left.arity !== "ternary" ||
        (!check_left(left.expression[1]) && !check_left(left.expression[2]))) &&
      (left.arity !== "binary" ||
        (id !== "." && id !== "?." && id !== "(" && id !== "["))
    ) {
  ...
  function enroll(name, role, readonly) {
    ...
    function_stack.forEach(function ({ context }) {
    ...
    if (earlier && id === "ignore") {
      ...
      warn("redefinition_a_b", name, id, earlier.line);
    } else if (option_dict.beta && global_dict[id] && role !== "parameter") {
      // test_cause:
    ...
    Object.assign(name, {
      dead: true,
      init: false,
      parent: role === "exception" ? catchage : functionage,
      readonly,
      role,
      used: 0,
    });
  ...
  function infix_dot(left) {
    ...
    if (
      ...
      (left.id !== "(regexp)" || (name.id !== "exec" && name.id !== "test"))
    ) {
      // test_cause:
  ...
  function prefix_function(the_function) {
    ...
    check_ordered(
      "function",
      function_list
        .slice(function_list.indexOf(the_function) + 1)
    ...
    function param_parse() {
      ...
      if (token_nxt.id === "{") {
        if (optional !== undefined) {
          ...
          warn("required_a_optional_b", token_nxt, token_nxt.id, optional.id);
        }
        param = token_nxt;
        param.names = [];
        advance("{");
  ...
  function prefix_lbrace() {
    ...
    if (token_nxt.id !== "}") {
      ...
      while (true) {
        name = token_nxt;
        advance();
        if ((name.id === "get" || name.id === "set") && token_nxt.identifier) {
    ...
    check_ordered(
      "property",
      the_brace.expression.map(function ({ label }) {
        return label;
      }),
    );
    advance("}");
  ...
  function stmt_if() {
    ...
    if (token_nxt.id === "else") {
      advance("else");
      the_else = token_now;
      the_if.else = token_nxt.id === "if" ? parse_statement() : block();
  ...
  function stmt_switch() {
    ...
    check_ordered_case(
      the_cases.map(function ({ expression }) {
        return expression[0];
      }),
    );
    dups = undefined;
    if (token_nxt.id === "default") {
      ...
      if (the_switch.else.length < 1) {
        ...
        the_last = the_switch.else[the_switch.else.length - 1];
...
function jslint_phase4_walk(state) {
  ...
  function post_t(thing) {
    if (
      ...
      (thing.expression[0].id === "||" || thing.expression[0].id === "&&")
    ) {
      // test_cause:
  ...
  function post_u(thing) {
    if (thing.id === "`") {
      if (
        thing.expression.every(function (thing) {
  ...
  function walk_expression(thing) {
    if (thing) {
      if (Array.isArray(thing)) {
        ...
        if (thing.arity === "preassign" || thing.arity === "postassign") {
          // test_cause:
...
function jslint_phase5_whitage(state) {
  ...
  function expected_at(at) {
    ...
    jslint_assert(!(right === undefined), `Expected !(right === undefined).`);
    warn(
      "expected_a_at_b_c",
      right,
      artifact(right),
        ...
        default:
          if (right.statement === true) {
            ...
            if (right.switch) {
              ...
              if (
                !open ||
                ((free || closer === "]") && left.line === right.line)
              ) {
                // test_cause:
...
/* have a sufficient contrast ratio. */
  ...
  functions.forEach(function (the_function) {
    ...
    html +=
      ...
      "<dfn>" +
      (id === "=>"
        ? "\u00ab" + htmlEscape(name) + "\u00bb" + htmlEscape(signature) + " =>"
        : (typeof name === "string"
            ? "\u00ab" + htmlEscape(name) + "\u00bb"
            : htmlEscape(name.id)) + htmlEscape(signature)) +
      "</dfn>";
    params = [];
    parameters.forEach(function extract({ id, names }) {
      switch (id) {
        case "[":
        case "{":
          // Recurse extract().
    ...
    html += detail(
      ...
      list.filter(function (id) {
        return (
          context[id].role === "variable" && context[id].parent === the_function
        );
      }),
    );
    html += detail(
    ...
    html += detail(
      ...
      list.filter(function (id) {
        return (
          context[id].closure === true && context[id].parent === the_function
        );
      }),
    );
    html += detail(
...
async function jstestDescribe(description, testFunction) {
  ...
  message =
    "\n  " +
    (Date.now() - jstestTimeStart) +
    "ms" +
    " - test describe - " +
    description +
    "\n" +
    result
      .map(function ([err, description, mode]) {
        jstestItCount += 1;
        if (err) {
          jstestCountFailed += 1;
          err =
...
async function moduleFsInit() {
  ...
  [moduleChildProcess, moduleFs, modulePath, moduleUrl] = await Promise.all([
...
function v8CoverageListMerge(processCovs) {
  ...
  function mergeTreeListToChildren(parentTrees) {
    ...
    function nextXxx() {
      ...
      let [nextOffset, nextTrees] = queueList[queueListIi] || [];
      let openRangeEnd;
      ...
      if (openRange === undefined) {
        ...
        nextTrees.forEach(function ({ parentIi, tree }) {
    ...
    queueList = Array.from(startToTreeDict)
      .map(function ([startOffset, trees]) {
        // new StartEvent().

        return [startOffset, trees];
      })
      .sort(function (aa, bb) {
        return aa[0] - bb[0];
      });
  ...
  function sortFunc(funcCov) {
    ...
    funcCov.ranges = treeToRanges(
      treeFromSortedRanges(funcCov.ranges.sort(compareRangeList)),
    );
    return funcCov;
  }
  ...
  processCovs.forEach(function ({ result }) {
  ...
  urlToScriptDict.forEach(function (scriptCovs) {
    ...
    scriptCovs.forEach(function ({ functions }) {
      functions.forEach(function (funcCov) {
        dictKeyValueAppend(
          ...
          funcCov.ranges[0].startOffset + ";" + funcCov.ranges[0].endOffset,
          funcCov,
        );
      });
    });
    rangeToFuncDict.forEach(function (funcCovs) {
      ...
      funcCovs.forEach(function (funcCov) {
        ...
        count +=
          funcCov.count !== undefined ? funcCov.count : funcCov.ranges[0].count;
  ...
  Object.entries(
    resultMerged.sort(function (aa, bb) {
      return aa.url > bb.url ? 1 : -1;
    }),
...
async function v8CoverageReportCreate({
  ...
  function htmlRender({ fileList, lineList, modeIndex, pathname }) {
    let html;
    let padLines;
    let padPathname;
    let txt;
...
<tbody>
    ...
    fileList.forEach(function (
      { linesCovered, linesTotal, modeCoverageIgnoreFile, pathname },
      ii,
    ) {
      let coverageLevel;
      let coveragePct;
      ...
      coverageLevel = modeCoverageIgnoreFile
        ? "coverageIgnore"
        : coveragePct >= 8000
          ? "coverageHigh"
          : coveragePct >= 5000
            ? "coverageMedium"
            : "coverageLow";
      coveragePct = String(coveragePct).replace(/..$/m, ".$&");
      if (modeIndex && ii === 0) {
        fill =
          // Badge-color rgb-red.

          "#" +
          Math.round((100 - Number(coveragePct)) * 2.21)
            .toString(16)
            .padStart(2, "0") +
          // Badge-color rgb-green.

          Math.round(Number(coveragePct) * 2.21)
            .toString(16)
            .padStart(2, "0") +
...
<text x="${xx1 + 0.5 * xx2}" y="14">${str2}</text>
      ...
      txt +=
        ...
        " | " +
        String(modeCoverageIgnoreFile + " " + coveragePct + " %").padStart(
          padLines,
          " ",
        ) +
        " | " +
      ...
      txt +=
        "| " +
        "*"
          .repeat(Math.round(0.01 * coveragePct * padPathname))
          .padEnd(padPathname, "_") +
        " | " +
        String(linesCovered + " / " + linesTotal).padStart(padLines, " ") +
        " | " +
        String(linesTotal - linesCovered + " / " + linesTotal).padStart(
          padLines,
          " ",
        ) +
        " |\n";
...
<div class="content">
            `).trim() + "\n";
      lineList.forEach(function ({ count, holeList, line, startOffset }, ii) {
        let chunk;
        let inHole;
        let lineHtml;
        let lineId;
        ...
        switch (count) {
          ...
          case 0:
            ...
            holeList.forEach(function ([aa, bb]) {
            ...
            chunk = "";
            line.forEach(function ({ char, isHole }) {
              if (inHole !== isHole) {
...
<div class="footer">
    ...
    consoleError("\n" + txt);
    promiseList.push(
      fsWriteFileWithParents(coverageDir + "coverage_report.txt", txt),
    );
  }
...
*/
  ...
  coverageDir = modulePath.resolve(coverageDir).replace(/\\/g, "/") + "/";
  ...
  while (processArgv[0] && processArgv[0][0] === "-") {
    ...
    switch (processArgElem[0]) {
      ...
      case "--include-node-modules":
        modeIncludeNodeModules = !/0|false|null|undefined/.test(
          processArgElem[1],
        );
        break;
    }
  ...
  if (processArgv.length > 0) {
    ...
    await Promise.all(
      Array.from(await moduleFs.promises.readdir(coverageDir)).map(
        async function (file) {
          if (/^coverage-\d+?-\d+?-\d+?\.json$/.test(file)) {
    ...
    exitCode = await new Promise(function (resolve) {
      ...
      moduleChildProcess
        .spawn(processArgv0, processArgv.slice(1), {
          ...
          shell: processArgv0.endsWith(".bat") || processArgv0.endsWith(".cmd"),
          stdio: ["ignore", 1, 2],
        })
        .on("exit", resolve);
    });
  ...
  v8CoverageObj = v8CoverageObj.filter(function (file) {
    return /^coverage-\d+?-\d+?-\d+?\.json$/.test(file);
  });
  v8CoverageObj = await Promise.all(
    v8CoverageObj.map(async function (file) {
      let data;
      ...
      data.result.forEach(function (scriptCov) {
        ...
        pathname = modulePath.resolve(pathname).replace(/\\/g, "/");
  ...
  await Promise.all(
    v8CoverageObj.result.map(async function ({ functions, url: pathname }) {
      let lineList;
      let linesCovered;
      let linesTotal;
      let source;
      ...
      lineList = [{}];
      source.replace(/^.*$/gm, function (line, startOffset) {
      ...
      functions.reverse().forEach(function ({ ranges }) {
          ...
          lineList.forEach(function (elem) {
            if (
              !(
                ...
                (startOffset <= elem.startOffset && elem.endOffset <= endOffset)
              )
            ) {
              return;
            }
            ...
            if (count === 0) {
              elem.count = 0;
              elem.holeList.push([startOffset, endOffset]);
            }
          });
        });
      });
      linesTotal = lineList.length;
      linesCovered = lineList.filter(function ({ count }) {
        return count > 0;
      }).length;
      ...
      fileDict[pathname] = {
        ...
        linesCovered,
        linesTotal,
        modeCoverageIgnoreFile: /^\/\*coverage-ignore-file\*\/$/m.test(
