...
module.exports = function (Posts) {
	...
	Posts.getReactionStatusByPostIDs = async function (pids, uid) {
		if (parseInt(uid, 10) <= 0) {
			...
			// YUKICHECK: forgot which var i changed the upvotes to... reacts or reactions??
	...
	async function fireReactHook(postData, uid, type, unreact, reactStatus) {
		const hook = unreact ? 'unreact' : 'react';
		const current = unreact ? 'react' : 'unreact'; // If unreacting, previous state was 'react'
